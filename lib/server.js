...
class Server extends events_1.EventEmitter {
    constructor(server, path, services, wsdl, options) {
        ...
        if (typeof path === "string" && path[path.length - 1] !== "/") {
            ...
            path.source[path.source.length - 1] !== "/") {
        ...
        wsdl.onReady((err) => {
            if (isExpress(server)) {
                ...
                server.addListener("request", (req, res) => {
                    ...
                    if (path === reqPath ||
                        (path instanceof RegExp && reqPath.match(path))) {
                        this._requestListener(req, res);
                    }
                    else {
    ...
    _process(input, req, res, cb) {
        ...
        const process = () => {
            ...
            try {
                ...
                const messageElemName = Object.keys(body)[0] === "attributes"
                    ? Object.keys(body)[1]
                    : Object.keys(body)[0];
    ...
    _envelope(body, headers, includeTimestamp) {
        ...
        if (includeTimestamp) {
            ...
            headers +=
                '<o:Security soapenv:mustUnderstand="1" ' +
        ...
        if (headers !== "") {
            xml += "<soapenv:Header>" + headers + "</soapenv:Header>";
        }
        xml += body
    ...
    _sendError(soapFault, callback, includeTimestamp) {
        ...
        if ("faultcode" in soapFault) {
            ...
            fault = this.wsdl.objectToDocumentXML("soapenv:Fault", soapFault, undefined);
        }
        else {
